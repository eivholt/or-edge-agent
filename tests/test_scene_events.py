...
def test_missing_suction_tip():
    data = json.loads((SCENARIOS_DIR / "missing_suction_tip.json").read_text())
    event = ORSceneEvent.model_validate(data)
    assert event.event_type == "or_setup_state_change"
    assert "suction_tip" in event.missing_or_uncertain
    assert "suction_tip" not in event.visible_items
